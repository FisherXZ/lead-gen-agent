...
def test_match_strong():
    ...
    projects = [
        {"id": "p1", "project_name": "Sunrise Solar", "state": "TX", "mw_capacity": 250, "developer": "SunDev"},
        {"id": "p2", "project_name": "Desert Wind", "state": "CA", "mw_capacity": 100, "developer": "WindCo"},
    ]
...
def test_match_deduplicates_projects():
    ...
    candidates = [
        SweepCandidate(project_name_hint="Sunrise Solar EPC contract", state="TX", epc_name="McCarthy", source_type="sec_edgar"),
        SweepCandidate(project_name_hint="Sunrise Solar construction site", state="TX", epc_name="McCarthy", source_type="osha"),
    ]
    projects = [
        {"id": "p1", "project_name": "Sunrise Solar", "state": "TX", "mw_capacity": 250, "developer": "SunDev"},
    ]
...
async def test_haiku_yes():
    ...
    candidate = SweepCandidate(project_name_hint="Sunrise Solar", epc_name="McCarthy", excerpt="McCarthy EPC for Sunrise Solar")
    project = {"project_name": "Sunrise Solar", "developer": "SunDev", "state": "TX", "mw_capacity": 250}
...
async def test_haiku_no():
    ...
    candidate = SweepCandidate(project_name_hint="Different Project", epc_name="McCarthy", excerpt="unrelated")
    project = {"project_name": "Sunrise Solar", "developer": "SunDev", "state": "TX", "mw_capacity": 250}
...
async def test_edgar_source_returns_candidates():
    mock_results = {
        "results": [
            {"snippet": "McCarthy EPC contract for Sunrise Solar 250MW", "url": "https://sec.gov/filing1"},
            {"snippet": "Another filing about solar construction", "url": "https://sec.gov/filing2"},
        ]
    }
...
async def test_osha_source_returns_candidates():
    mock_results = {
        "results": [
            {"employer_name": "McCarthy Building", "address": "Pecos, TX, 79772", "state": "TX", "inspection_date": "2025-06-15", "detail_url": "https://osha.gov/1"},
        ]
    }
...
async def test_sweep_no_epcs_returns_early():
    ...
    mock_client.table.return_value.select.return_value.contains.return_value.order.return_value.execute.return_value = MagicMock(data=[])
...
async def test_sweep_processes_epc_with_mocked_sources():
    ...
    projects_resp.data = [
        {"id": "p1", "project_name": "Sunrise Solar", "state": "TX", "mw_capacity": 250, "developer": "SunDev", "county": "Travis", "latitude": 30.27, "longitude": -97.74},
    ]
    ...
    def table_side_effect(name):
        ...
        if name == "entities":
            mock_table.select.return_value.contains.return_value.order.return_value.execute.return_value = entities_resp
        elif name == "projects":
            mock_table.select.return_value.is_.return_value.order.return_value.limit.return_value.execute.return_value = projects_resp
        elif name == "epc_discoveries":
            mock_table.insert.return_value.execute.return_value = MagicMock(data=[{"id": "disc-new"}])
        return mock_table
...
async def test_sweep_skips_existing_discovery():
    ...
    entities_resp = MagicMock()
    entities_resp.data = [{"id": "e1", "name": "McCarthy", "entity_type": ["epc"], "metadata": {}, "aliases": []}]

    projects_resp = MagicMock()
    projects_resp.data = [{"id": "p1", "project_name": "Sunrise Solar", "state": "TX", "mw_capacity": 250, "developer": "SunDev", "county": "Travis", "latitude": None, "longitude": None}]
    ...
    class MockSource:
        ...
        async def search(self, epc_name, **kwargs):
            return [SweepCandidate(
                ...
                state="TX", epc_name=epc_name, source_type="mock",
            )]
