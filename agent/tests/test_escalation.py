...
def _tool_result_msg(name, content):
    return {
        "role": "user",
        "content": [{"type": "tool_result", "tool_use_id": f"id-{name}", "content": content}],
    }
...
def _assistant_msg(name):
    return {
        "role": "assistant",
        "content": [{"type": "tool_use", "id": f"id-{name}", "name": name, "input": {}}],
    }
...
def test_continue_under_limits():
    ...
    msgs = [
        ...
        _tool_result_msg(
            "web_search", json.dumps({"results": [{"title": "SunPower EPC project"}]})
        ),
    ]
...
def test_stagnation_user_mode():
    p = EscalationPolicy(
        ...
        min_iterations_before_stagnation=0,
    )
    msgs = []
    for _ in range(4):
        msgs += [
            _assistant_msg("web_search"),
            _tool_result_msg("web_search", json.dumps({"results": []})),
        ]
...
def test_no_stagnation_with_signals():
    ...
    for co in ["SunPower Corp", "NextEra Energy", "Blattner Energy", "Mortenson Construction"]:
        msgs += [
            _assistant_msg("web_search"),
            _tool_result_msg("web_search", json.dumps({"results": [{"title": f"{co} wins EPC"}]})),
        ]
...
def test_consecutive_errors():
    ...
    for _ in range(3):
        msgs += [
            _assistant_msg("web_search"),
            _tool_result_msg("web_search", json.dumps({"error": "500"})),
        ]
