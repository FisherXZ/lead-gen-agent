...
async def test_injects_conversation_id_for_remember():
    action = await InjectContextHook().pre_tool(
        "remember", {"fact": "test"}, _ctx(conversation_id="conv-42")
    )
...
async def test_injects_conversation_id_for_recall():
    action = await InjectContextHook().pre_tool(
        "recall", {"query": "epc"}, _ctx(conversation_id="conv-99")
    )
...
async def test_injects_session_id_for_manage_todo():
    action = await InjectContextHook().pre_tool(
        "manage_todo", {"action": "list"}, _ctx(session_id="sess-7")
    )
...
async def test_injects_session_id_for_research_scratchpad():
    action = await InjectContextHook().pre_tool(
        "research_scratchpad", {"data": "x"}, _ctx(session_id="sess-3")
    )
...
async def test_does_not_overwrite_existing_session_id():
    action = await InjectContextHook().pre_tool(
        "manage_todo", {"action": "list", "session_id": "custom"}, _ctx(session_id="sess-7")
    )
