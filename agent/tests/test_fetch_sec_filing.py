...
async def test_cik_plus_accession_builds_correct_url():
    ...
    assert url == "https://www.sec.gov/Archives/edgar/data/0000003153/000156459025012345/smxt-20250805.htm"
...
async def test_html_filing_extraction():
    ...
    with patch("src.tools.fetch_sec_filing.cache_get", return_value=None), \
         patch("src.tools.fetch_sec_filing.cache_set"), \
         patch("src.tools.fetch_sec_filing._fetch_with_retry", return_value=mock_response), \
         patch("trafilatura.extract", return_value="SolarMax Technology awarded $127.3 million EPC contractor agreement for 430 MWh battery storage project in Texas."):
...
async def test_cache_hit():
    ...
    cached = {"url": "https://sec.gov/test.htm", "text": "cached content", "source_type": "sec_filing"}
...
async def test_404_returns_error():
    ...
    with patch("src.tools.fetch_sec_filing.cache_get", return_value=None), \
         patch("src.tools.fetch_sec_filing._fetch_with_retry",
